...
class spmean:
    ...
    # The dataMean() function will compute the mean of a set of data column between dates.
                    ...
                    if filename.endswith(".txt"):
                        ...
                        def avgCalc(x, y):
                            ...
                            for i in range(len(data_file)):
                                if (np.char.replace(data_file[i, 0], "/", "").astype(
                                        np.float) > x and np.char.replace(
            ...
            if filename.endswith(".txt"):
                for file in os.listdir(directory):
                    ...
                    if filename.endswith(".txt"):
                        ...
                        data_file = np.loadtxt(self.directory_str + filename, dtype=str, delimiter=',')
                        for i in range(len(data_file)):
                            tempArr.append(float(data_file[i, 4]))
